...
from rfo_demo.data import DarcyDataset
from rfo_demo.models import get_model_class
from rfo_demo.optim import Adam
from rfo_demo.utils import plot_training_metrics, setup_logger
from rfo_demo.visualization import plot_2d_results


TRAIN_DATA_PATH = "/data5/store1/dlt/rectified_flow/data/piececonst_r421_N1024_smooth1.mat"
TEST_DATA_PATH = "/data5/store1/dlt/rectified_flow/data/piececonst_r421_N1024_smooth2.mat"


def _build_loaders(batch_size: int, target_len: int):
    train_dataset = DarcyDataset(TRAIN_DATA_PATH, mode="train", target_size=target_len)
    test_dataset = DarcyDataset(TEST_DATA_PATH, mode="test", target_size=target_len)
...
def run(config: dict) -> None:
    ...
    train_dataset, test_dataset, train_loader, _ = _build_loaders(batch_size, target_len)
    ...
    score_net = model_class().to(device)
    ...
    with torch.no_grad():
        ...
        yt_last = torch.where(
            yt_last < threshold,
            torch.tensor(3.0, device=yt_last.device, dtype=yt_last.dtype),
            torch.tensor(12.0, device=yt_last.device, dtype=yt_last.dtype),
        )
        y_true = a * a_std + a_mean
        tolerance = 1e-6
        match_mask = torch.isclose(yt_last, y_true, atol=tolerance)
        classification_acc = match_mask.sum().item() / yt_last.numel()
        final_l2rel = calculate_l2_relative_error(xt_last, x_true)
        final_l2rel_reverse = calculate_l2_relative_error(yt_last, y_true)
        logging.info("Final evaluation - forward L2 Relative Error: %.6f", final_l2rel)
        logging.info("Final evaluation - reverse L2 Relative Error: %.6f", final_l2rel_reverse)
        logging.info("Final evaluation - classification accuracy: %.6f", classification_acc)
        plot_2d_results(data1=x_true, data2=xt_last, labels=["Ground Truth", "Prediction"], base_filename=f"{save_path}{scorenet_model_class.lower()}_{target_len}_operator_learning_1d")
        plot_2d_results(data1=y_true, data2=yt_last, labels=["Ground Truth", "Prediction"], base_filename=f"{save_path}{scorenet_model_class.lower()}_{target_len}_reverse_learning_1d")
