...
class burgerDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, train_ratio=0.9, mode='train'):
        """简化版burger流数据集，直接返回原始数据"""
        self.file_path = file_path
        self.reader = MatReader(file_path)
        
        # 读取输入输出数据
        self.inputs = self.reader.read_field('a')  
        self.outputs = self.reader.read_field('u')    
...
def main():
    # 替换为实际数据文件路径
    file_path = "/data5/store1/dlt/rectified_flow/data/burgers_data_R10.mat"
    
    # 创建数据集实例（训练集）
    train_dataset = burgerDataset(
        file_path=file_path,
        train_ratio=0.9,
        mode='train'
    )
    ...
    print(f"测试集大小: {len(burgerDataset(file_path, mode='valid'))}")
